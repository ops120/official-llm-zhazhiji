...
### 访问验证触发频率（真机实录 2026-09-12，样本 n=8 发）

- **触发点是对话流「发送消息」**：导航 / 浏览类操作（doctor、list-models、打开画图视频工作流页）全程零触发；
- 游客对话流：**静置后第 1、2 发通过（距上发 4–9 分钟），第 3 发起连发全弹**（间隔 4–5 分钟无效）；
- 进入风控态后每个新会话都弹、**真人拖动也大概率失败**（指纹被拉黑），持续重试只会延长风控时间；
  停发约 40 分钟后仍弹——冷却 >40 分钟，宁等勿试；
- 工作流页（画图/视频）连发 6 次生成零触发；登录态工作流 10 次会话零触发（对话流登录态样本不足）；
- 详细数据表与恢复策略见 [README.md](README.md)「风控触发频率」。
...
| 产物 | 4 张图（一次生成） | 1 个 mp4（`sfile.chatglm.cn/api/cogvideo/*.mp4`，实测 5s 档 1280×720） |
| 耗时 | 约 20–30 秒 | 约 60–90 秒（排队 + 13%→100% 进度） |
| 参数控件 | 快速 / 风格 / 1:1 / 咒语 | 首帧上传 / 通用生成 / 基础参数 / 5s / AI 音效 / 去水印 |
| 发送按钮 | composer 内 `div.enter` | **`div.btn-group.score-visible`**（纸飞机 svg + 余额文字；初始 disabled，输入后激活） |
...
另两个实测坑：① **生成结果会进「创作历史」标签页而非内联展示**——只盯主区会误判超时
（实测踩过：任务实际已成功，切到创作历史才看到 mp4）；
② 视频页发送按钮是 `btn-group` 容器（内含纸飞机 svg + 「XX积分」余额文字），
点参数行（通用生成 / 基础参数 / 5s…）的中心只会打开设置，不会发送。

### 额度实测（2026-09-12）

> ⚠️ **读余额的坑**：侧栏「积分 X.XX」在 SPA 内导航时**不会刷新**，只有整页加载才更新——
> 会话内连读两次余额一样，不代表没有扣费（实测踩过，差点得出「生图免费」的错误结论）。
> 自动化读余额必须整页 reload 后再取。

**计费模型（真机验证）**：

- **生图（快速模式）= 免费**，不扣积分（对照实验：整页刷新前后余额 100.00 → 100.00，
  期间成功生成 1 轮 4 张；另有两轮同样零扣费）。`GLM Image new` 模式未测。
- **生视频消耗积分，价格标注在「生成时长」菜单里**：**5s = 50 积分/条、10s = 150 积分/条**
  （AI 音效 / 去水印的加价未测）。对账：一轮会话成功生成 2 条 5s，余额 200.00 → 100.00，正好 2×50。
- ⚠️ 发送按钮上的「XX积分」文字是**当前余额**，不是单价（极易误读）；真实单价以时长菜单标注为准。

**游客态**：

- **生图**：游客连续生成 **5 轮**成功（每轮 4 张 = 20 张，实测均为 1024×1024 JPEG，单轮 15–23 秒），
  **第 6 轮点生成即弹登录墙**（微信扫码 / 手机号+短信），事前没有任何余额提示。
  即：**游客生图上限 ≈ 5 轮 / 20 张 / 天**（是否按日重置未验证；游客生图与积分无关，是独立次数额度）。
  自动化时应把「点生成后弹出登录弹窗」当作额度到顶信号——**停下并告知用户需登录，不要重试**。
- **生视频**：游客**完全不可用**——进入 `/video` 页即弹「请输入短信验证码」（手机验证 / 登录墙），
  composer 无法获得焦点，连参数都看不到。
