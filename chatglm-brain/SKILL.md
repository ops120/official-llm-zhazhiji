...
**登录态（积分制，登录账号实测余额 200.00 积分）**：

- **生视频（清影）**：默认档 **5s / 1280×720**（实测 3 个成品均为 5.2s、720p、1.5–4.3 MB mp4，
  `avc1` 编码）。测试期间余额 200.00 → 100.00，**消耗 100 积分 / 3 个视频 ≈ 33 积分/个**
  （精确单价未单测验证）。生成入口还有 10s 档、AI 音效、去水印、AI 特效、基础参数可选，
  这些档位的加价未实测。
- ⚠️ **生视频的结果进「创作历史」标签页，不一定内联展示**——自动化检测产物必须扫历史区
  （按新增 cogvideo mp4 URL 判定），只盯页面主区会误判超时（实测踩过：任务实际已成功）。
- ⚠️ 发送按钮是 composer 最右侧的**纸飞机图标**；`btn-group` 是参数行（通用生成/基础参数/5s…），
  点它的中心会误触参数控件而非发送。
- **登录后生图额度**：未实测（积分对生图的扣费规则待验证；游客期数据见上）。

> 积分余额显示在侧栏用户名下方（`积分 200.00`），自动化可直接读取作为额度信号。
