...
| **风控实际发生** | 零触发 | 零触发 | 零触发 | 零触发 | 首次配置时 1 次验证码（人工通过） |
| **版本** | 3.0.0 | 3.0.0 | 3.0.0 | 3.0.0 | 3.0.1 |

点上面的仓库名进各自的 README，有完整的能力表、命令面、返回值契约与「真机验证过的坑」。

> 表中能力、模型名与分辨率均为编写时实测值；网页版功能会随站点调整，**实际以页面显示为准**。

> chatglm-brain（开发中的第六个，暂未入表）在连续请求下多次弹滑块「访问验证」（2026-09-12 记录，均按 skill 规则提醒并由用户本人拖动通过）。
