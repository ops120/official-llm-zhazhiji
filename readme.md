...
机队现有四台榨汁机，各自独立仓库、独立 skill，互不依赖
（本仓库通过 git submodule 引用它们，四个 brain 的 git 历史完全独立）。
**四个只是当前规模，不是固定上限**：后续会按实际情况增加新大脑、持续更新已有大脑，
本文档与各章节随之维护：

| | [deepseek-brain](https://github.com/ops120/deepseek-brain) | [doubao-brain](https://github.com/ops120/doubao-brain) | [gemini-brain](https://github.com/ops120/gemini-brain) | [grok-brain](https://github.com/ops120/grok-brain) |
| --- | --- | --- | --- | --- |
| **CLI** | `dsb` | `dbb` | `gmb` | `grb` |
| **官网** | chat.deepseek.com | doubao.com | gemini.google.com | grok.com |
| **独有能力** | 深度思考 + 智能搜索 | 生图 / **生视频** / 音乐 / 播客 / 录音转写 | 生图（2816×1536 原图）+ 代码 Canvas | **X 平台实时信息** + 多档模型 |
| **模型可选** | ✗（只有「深度思考」「智能搜索」两个开关） | ✓（快速 / 2.1 Turbo） | ✓（Flash-Lite / Flash / Pro） | ✓（Fast / Build 可用；Auto / Expert / Heavy 需订阅） |
| **登录持久化** | 简单 | 简单（字节系 cookie 是持久型） | **复杂**（session cookie，需三重保险） | 简单（持久型 cookie） |
| **版本** | 3.0.0 | 3.0.0 | 3.0.0 | 3.0.0 |
...
四个仓库的 `scripts/<cli>/src/` **文件构成相同**（文件名一一对应），把「能复用的」和「站点专属的」切得很干净：

| 文件 | 角色 |
| --- | --- |
| `sanitize.mjs` | 发送前的确定性净化闸门（四仓文件一致） |
...
- **人工登录一次，之后尽量复用**：登录/人机验证只在网站重弹时才打扰你（`LOGIN_REQUIRED` /
  ...
  四仓的登录持久化难度不同：DeepSeek、豆包与 Grok 的 cookie 是持久型，基本一劳永逸；
  **Gemini 依赖 session cookie，站点风控或会话过期时会要求重新登录**。
  登录判定的可靠依据是 cookie（Grok 走 `sso`/`sso-rw`/`x-userid`，且未登录时
  「界面也有输入框」——看界面会误判）。
- **协作协议**（`[DSB]` / `[DBB]` / `[GMB]` / `[GRB]`）：让大脑做 PLAN → 你执行 → 它 REVIEW 的循环，
...
- **状态目录隔离**：回答正文默认不落盘（只记元数据；例外是 `--debug` 或失败时的 `debug/` 快照，
  会含未脱敏正文，排障后请删除）；cookie（以及 Gemini / 豆包 / Grok 额外保存的
...
> 真正要安装的是下面四个子仓库，按你需要的能力任选其一或多选。

把子仓库 clone 到宿主的 skills 目录即可（仓库内部无需改任何路径；
命令行入口还要按下一节配别名或用完整路径）。
下例以 deepseek-brain 为例，`doubao-brain` / `gemini-brain` / `grok-brain` 换个名字同理。
...
> **clone 完还不能直接运行命令**：下文 `dsb` / `dbb` / `gmb` / `grb` 是文档简写，不是安装出来的可执行文件。
> 用之前必须先配别名（见本节下方的「关于命令写法」），或把示例里的简写替换成完整 `node "..."` 路径。

只看项目结构、做二次开发才需要主仓库（四个 brain 会作为 submodule 一起拉下来）：

```bash
git clone --recursive https://github.com/ops120/official-llm-zhazhiji.git
```

> 注意：`--recursive` 拉下来的四个 brain **不会被 agent 自动发现**，
> 要让 agent 用上仍需把它们（或其副本）放进宿主的 skills 目录。

装好后对 agent 说：**「用 deepseek-brain 完成首次配置」**（换 `doubao-brain` / `gemini-brain` / `grok-brain` 同理），
agent 会替你跑 `setup`。也可以自己手动执行首次配置（把路径换成你实际的安装位置）：

```bash
node "$SKILLS_DIR/deepseek-brain/scripts/dsb/cli.mjs" setup   # 换 dbb / gmb / grb 与目录名同理；$SKILLS_DIR 见下节
```

首次配置会检查环境、把 `playwright-core` 装到状态目录、打开有头浏览器**请你本人登录**，然后冒烟验证。
状态目录被删除或迁移后，需要重新跑一次 `setup`（依赖与登录态都在那里）。

> **关于命令写法（重要）**：下文 `dsb` / `dbb` / `gmb` / `grb` 都是**文档简写**，并非安装好的命令。
...
四个 brain 都是 skill，**装好之后不需要记命令**——直接对 agent 说人话即可。
...
- **首次要先配置**：四个 brain 各需人工登录一次（见「安装」），之后基本不用管。
...
四个 CLI 的公共命令面同构（`setup` / `login` / `logout` / `doctor` / `ask` / `thread` / `session` / `logs` / `update-check`）；
`list-models` 仅 doubao、gemini 与 grok 有（DeepSeek 网页版没有模型选择器，故无此命令）。
...
> 状态目录位置（`<name>` 取 `deepseek` / `doubao` / `gemini` / `grok`；覆盖变量见表格下方）：
>
> | 系统 | 路径 |
> | --- | --- |
...
>
> 覆盖变量：`DSB_STATE_DIR`（deepseek-brain）/ `DBB_STATE_DIR`（doubao-brain）/ `GMB_STATE_DIR`（gemini-brain）/ `GRB_STATE_DIR`（grok-brain）。
...
四个子目录是 **git submodule**：各自指向独立仓库、各自保留完整 git 历史；
...
四个不是上限；后续要加新大脑时（第五个、第六个……同理），复制一个现有仓库即可。
...
- `browser.mjs` —— 浏览器探测与**登录持久化**（不同站点的 cookie 策略可能不同，
  参考 gemini-brain 的「三重保险」与 doubao-brain / grok-brain 的持久型 cookie 结论）
- `site.mjs` —— 选择器、输入注入、完成判定、内容/产物抽取（站点专属，**不能直接跑**）

## 许可证

本项目基于 MIT License 开源，完整条款见 [LICENSE](LICENSE)；
四个子仓库各自独立遵循 MIT，许可证文件见：
